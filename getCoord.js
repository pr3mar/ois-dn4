...
function theNext() {
    if (nextAddress < lekarne.length) {
        setTimeout('getCoord("' + lekarne[nextAddress].naslov + '",theNext)', 800);
        nextAddress++;
    } else {
        // We're done. Show map bounds
        console.log("finish");
        //console.log(lekarne);
        /*var url = 'data:text/json;charset=utf8,' + encodeURIComponent(lekarne.seznam);
        window.open(url, '_blank');
        window.focus();*/
        $("#rez").append("<div id='json'>" + JSON.stringify(lekarne) + "</div>");
    }
}
